...
while true
do
    # if .version file exists, read it and set the version variable
    if  [[ -f .version ]]; then
        sed -i -e "s/motd=.*/motd=All the Mods 9 v$(cat .version)/" server.properties
    fi

    exec "${ATM9_JAVA:-java}" @user_jvm_args.txt @libraries/net/minecraftforge/forge/1.20.1-$FORGE_VERSION/unix_args.txt nogui
